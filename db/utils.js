...
const getAttrs = (model) => {
    logger.debug('db.utils.getAttrs. Get attributes for model', model.modelName)
    ...
    logger.debug('db.utils.getAttrs. The attributes are:', props)
    return props  
}

const getRefs = (model) => {
    logger.debug('db.utils.getRefs. Get refs for model', model.modelName)
    const virtualKeys = Object.keys(model.schema.virtuals).filter((key) => key !== 'id')  // ignore id
    logger.debug('db.utils.getRefs. The keys for virtual:', virtualKeys)
    const refs = []
    ...
    logger.debug('db.utils.getRefs. refs = ', refs)
    return refs
}

const getAttrsByModelName = (name) => {
    logger.debug('db.utils.getAttrsByModelName. Get attributes for model', name)
    const model = mongoose.model(name)
    let props = Object.keys(model.schema.paths)
    props = props.filter((attr) => attr !== '__v').map((attr) => (attr === '_id') ? 'id' : attr) 
    logger.debug('db.utils.getAttrsByModelName. The attributes are:', props)
    return props 
}
